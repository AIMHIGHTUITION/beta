<?php 
	...
	if ( $exmtle == "Learning Style" ) {
		...
		//check if student has exam in temp.gen.exam table
		$sqlHasExam = "SELECT b.id as question_id, b.questions as questions, a.answer as answer,a.time_to_answer as time_to_answer from tbltemporarygenexam as a left outer join learning_style_questions as b on a.question_id=b.id where a.student_id='".$studenId."' and assessment_type_id='".$asstype."' and a.status is null";
		//count the total questions that the student answered
		$sqlDone = "select count(a.question_id) as donePrimary from tbltemporarygenexam as a left outer join learning_style_questions as b on a.question_id=b.id where a.student_id='".$studenId."' and assessment_type_id='".$asstype."' and a.status is not null and question_type='primary'";
		$rowsDone = $db->query($sqlDone);
		$resultDone = $db->fetch_array($rowsDone);
		//count the total number of questions in primary
		$sqlallQuestion = "Select count(a.question_id) as totalPrimary from tbltemporarygenexam as A left outer
 		join learning_style_questions as b on a.question_id=b.id where a.student_id='".$studenId."' and assessment_type_id='".$asstype."' and question_type='primary'";
		...
		while($recordGenExam = $db->fetch_array($rowHasExam)){
			...
			(
				...
				'question' => utf8_encode($recordGenExam["questions"]),
		...
		//TODO: Add Where if diagnostic then get question from diagnostic
			...
			$sql = "SELECT id from learning_style_questions WHERE id<=6";
	...
	} elseif ($exmtle == "Maths Diagnostic" || $exmtle == "Maths Extensions") {
